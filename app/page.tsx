...
export default function Home() {
  ...
  const filteredPoints = useMemo(() => rows.filter((row) => {
    const haystack = `${idOf(row)} ${nameOf(row)} ${auditorOf(row)} ${channelOf(row)} ${value(row, ["RUTA PREVENTA"])}`.toLowerCase();
...
function RouteDetailPanel({ auditor, rows, onClose }: { auditor: string; rows: Row[]; onClose: () => void }) {
  const mapped = rows.map((row, index) => ({ row, index, point: coordinatesOf(row) })).filter((item): item is { row: Row; index: number; point: { lat: number; lng: number } } => Boolean(item.point));
  const bounds = mapped.length ? { minLat: Math.min(...mapped.map((item) => item.point.lat)), maxLat: Math.max(...mapped.map((item) => item.point.lat)), minLng: Math.min(...mapped.map((item) => item.point.lng)), maxLng: Math.max(...mapped.map((item) => item.point.lng)) } : null;
  return <div className="detail-overlay"><div className="detail-header"><div><p>RUTA DEL DÍA</p><h1>{auditor}</h1><span>{rows.length} puntos programados</span></div><button onClick={onClose}>← Volver a rutas</button></div><div className="detail-actions">{routeChunks(rows).map((chunk, index) => <a key={index} href={mapsRouteUrl(chunk)} target="_blank" rel="noreferrer">Abrir {Math.ceil(rows.length / 25) === 1 ? "ruta en Maps" : `tramo ${index + 1}`} ↗</a>)}<small>Google Maps permite hasta 25 paradas por tramo.</small></div><div className="detail-layout"><section className="detail-map"><div className="detail-map-title"><b>Mapa de puntos</b><span><i className="fixed-dot"></i> Fijo <i className="regular-dot"></i> No fijo</span></div>{bounds && mapped.map(({ row, index, point }) => { const left = 8 + ((point.lng - bounds.minLng) / Math.max(bounds.maxLng - bounds.minLng, .01)) * 84; const top = 87 - ((point.lat - bounds.minLat) / Math.max(bounds.maxLat - bounds.minLat, .01)) * 75; return <a className={fixedOf(row).toUpperCase() === "SI" ? "detail-marker fixed" : "detail-marker regular"} style={{ left: `${left}%`, top: `${top}%` }} href={pointMapsUrl(row)} target="_blank" rel="noreferrer" title={nameOf(row)} key={`${idOf(row)}-${index}`}>{index + 1}</a>; })}</section><section className="detail-points">{rows.map((row, index) => <article key={`${idOf(row)}-${index}`}><span className={fixedOf(row).toUpperCase() === "SI" ? "point-number fixed" : "point-number regular"}>{index + 1}</span><div><strong>{nameOf(row)}</strong><small>{idOf(row)} · Ruta {value(row, ["RUTA PREVENTA"]) || "—"}</small><p><b>{channelOf(row) || "Sin canal"}</b><em className={fixedOf(row).toUpperCase() === "SI" ? "is-fixed" : "is-regular"}>{fixedOf(row).toUpperCase() === "SI" ? "PDV fijo" : "PDV no fijo"}</em></p></div><a href={pointMapsUrl(row)} target="_blank" rel="noreferrer">Ver en Google Maps ↗</a></article>)}</section></div></div>;
}
